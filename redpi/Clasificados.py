...
def consulta_sfe(fecha):
    ...
    try:
        ...
        + "and enviado_at >= '"+fecha+" 01:59:00.0' and enviado_at <= '"+fecha+" 14:59:00.0'")
        row=cursor.fetchall()
        pagosSFE = []
...
def consulta_caja(fecha):
    try:
        ...
        for i in row:
            ...
            try:
                pagosCJ.append({'fecha':i[3],'recibo':i[0],'expediente':i[2],'status':i[1],'ftabla':format_fech,'tipo_movimiento':consulta_Fop_out(str(i[2]))[0]['tipo_movimiento'],'movimiento':consulta_Fop_out(str(i[2]))[0]['movimiento'],'tipo_solicitud':consulta_Fop_out(str(i[2]))[0]['tipo_solicitud'],'fecha_pago':consulta_Fop_out(str(i[2]))[0]['fecha_pago'],'fecha_inicio':consulta_Fop_out(str(i[2]))[0]['fecha_inicio'],'fecha_fin':consulta_Fop_out(str(i[2]))[0]['fecha_fin']}) 
            except Exception as e:
...
#inicio y fin a la publicacion emitida en form_orden_publicacion
def update_inicio_fin(exp,tipo,pago):   
        try:    
...
#Insert detalle_clasificados
def insert_clasificado(exp,userp):
        try:
            ...
            try:
                ...
                cursor = conn.cursor()
                cursor.execute("INSERT INTO public.detalle_clasificado (num_orden, fecha_solicitud, hora_solicitud, tipo_solicitud, tipo_signo, tipo_marca, clase, denominacion, solicitante, direccion, pais, agente, descripcion, logo, expediente, nom_agente, user_login, edicion, estado, inicio, fin, fecha_pago, fec_reg, process_user) \n"                                                                              ##Hora Inicio##
                            +" VALUES('0', '"+fecha_exp[0]+"', '"+hora_exp[0]+"', '"+str(respuesta['applicationType'])+"', ' "+str(tipo_signo)+"', '"+str(tipo_signo)+"', '"+str(clase[0])+"', '"+str(marca)+"', '"+str(nombre)+"', '"+str(direccion).replace("'","´")+"', '"+str(pais)+"', '"+str(agente)+" -  "+str(agente_nombre)+"', 'DESCRIPCION ', '"+str(logo_m_mark)+"', "+str(expediente[0])+", '', '', '', '1', '"+str(fecha_mañana)+"', '"+str(fin_pub)+"', '','','"+userp+"');")    
                cursor.rowcount
                conn.commit()
                conn.close()               
                return('listo')
...
def insertar_edicion(fecha,edicion):
    lista_exp = []
    lista_id = []
    edicion_id = 0
    try: 
        ...
        #print(fecha)
        #print(str(dia_tras).strip())
        #print(str(dia_tras_2).strip())
        #print(str(dia_tras_3).strip())
        #print(str(dia_tras_4).strip())
