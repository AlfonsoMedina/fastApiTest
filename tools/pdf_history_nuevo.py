...
def traer_data(exp):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("select * from me_publicaciones.new_ordenes_publicaciones where estado = 2 and expediente = '"+exp+"'")    
        row=cursor.fetchall()
        for i in row:
            ...
            def firma():
                ...
                pdf.cell(w=0, h=7, txt='', border='LR',ln=1)
                #Imagen firma
                pdf.image(str(i[12]),x=140,y=(pdf.get_y() -7),w=35)
                pdf.cell(w=0, h=20, txt='', border='LR',ln=1)
                pdf.set_font('Arial', '', 8)
                pdf.set_font(style="I")
                pdf.cell(w=0, h=8, txt='Debe llevar la firma del funcionario autorizado y el sello oficial      ',border='LBR', ln=1,align='R')
            ...
            pdf.set_font('Arial', '', 10)
            pdf.multi_cell(w=0, h=20, txt= unicodedata.normalize('NFKD', str(i[3])).encode('ascii', 'ignore').decode().strip(), border=1, align='c',ln=1)
            ...
            #pdf.multi_cell(w=130,h=4, align='L', txt='Detalle:'+str(i[18]), border=0)
